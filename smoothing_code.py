...
def smooth_variance_map(II, QQ, UU, IQ=None, IU=None, QU=None, fwhm_out=1.0,  Bl=None, lmax=None, nside_out=None):
    ...
    dQQ[II_mask] = hp.UNSEEN
    dUU[II_mask] = hp.UNSEEN
    dIQ[II_mask] = hp.UNSEEN
    dIU[II_mask] = hp.UNSEEN
    dQU[II_mask] = hp.UNSEEN

    dII[~II_mask] *= PIXAREA * (2*np.pi)
    dQQ[~II_mask] *= PIXAREA * (2*np.pi)
    dUU[~II_mask] *= PIXAREA * (2*np.pi)
    dIQ[~II_mask] *= PIXAREA * (2*np.pi)
    dIU[~II_mask] *= PIXAREA * (2*np.pi)
    dQU[~II_mask] *= PIXAREA * (2*np.pi)

    return dII, dQQ, dUU, dIQ, dIU, dQU 


def smooth_variance_map_with_rotation(II, QQ, UU, IQ=None, IU=None, QU=None, fwhm_out=1.0,  Bl=None, lmax=None, nside_out=None):
    """
    Smooth input variance maps.

    Smooths two versions of the variance map, one in the original coordinate system and one rotated 90 degrees. 
    The two maps are then combined to produce the final variance map
